...
const ProjectsPage = () => {
  ...
  const [projectList, setProjectList] = useState([]); // ✅ no shadowing
  ...
  useEffect(() => {
    ...
    setProjectList(getProjects()); // ✅ lazy load projects
  ...
  const filteredAndSortedProjects = useMemo(() => {
    ...
    switch (sortBy) {
      ...
      case "newest":
        return [...filtered].reverse(); // assumes seed is oldest→newest
      default:
        return filtered;
    }
  }, [activeCategory, searchQuery, sortBy, projectList]); // ✅ include projectList
  ...
  const handleOpenModal = useCallback(
    (index) => console.log("Open modal for project:", index),
    []
  );
  ...
  return (
    ...
    >
      ...
      >
        ...
        >
          <p>
            Showing {filteredAndSortedProjects.length} of {projectList.length}{" "}
            projects
            {(searchQuery || activeCategory !== "All") && (
