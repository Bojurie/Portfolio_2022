...
// Trust proxy so secure cookies & IPs work correctly behind reverse proxies (Render/NGINX/etc.)
app.set("trust proxy", 1);

/* ------------------------------- CORS ----------------------------------- */
/**
 * CORS allowlist:
 * - CORS_ORIGIN env supports comma-separated origins: "http://localhost:3000,https://yourdomain.com"
...
const corsOptions = {
  origin(origin, cb) {
    // Allow same-origin requests or tools (no origin)
...
app.use((req, res, next) => {
  // Ensure correct Vary so caches don’t mix origins
  res.vary("Origin");
  next();
});
