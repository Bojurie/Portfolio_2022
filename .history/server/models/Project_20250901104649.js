...
const ShortVideoSchema = new mongoose.Schema(
  {
    ...
    poster: { type: String, trim: true }, // optional poster (client uses "poster")
    duration: { type: Number, min: 0, max: 90 }, // keep “short” by policy
    width: Number,
    height: Number,
    bytes: Number,
  },
...
const projectSchema = new mongoose.Schema(
  {
    ...
    // Useful for your client links
    links: {
      live: { type: String, trim: true },
      code: { type: String, trim: true },
    },

    // Reaction counters (fast reads)
    reactionCounts: {
...
/* ---------- Reactions backfill (matches Reaction.targetType='project') ---------- */
projectSchema.statics.recalcReactionCounts = async function (projectId) {
  ...
  const rows = await Reaction.aggregate([
    {
      $match: {
        targetType: "project", // ✅ lowercase, consistent with your Reaction model
