...
const ShortVideoSchema = new mongoose.Schema(
  {
    ...
    duration: { type: Number, min: 0, max: 90 }, // short form
    width: Number,
    height: Number,
    bytes: Number,
  },
    ...
    tags: {
      ...
      set: (arr) =>
        (Array.isArray(arr) ? arr : [])
          .map((t) => String(t).trim())
          .filter(Boolean)
          // keep original case for UI filters; dedupe
          .filter((t, i, a) => a.indexOf(t) === i),
    },

    // up to 3 images (enforced in controller + validated here)
    images: {
      type: [ImageSchema],
      default: [],
      validate: {
...
// Recalculate reaction counters from Reaction collection
